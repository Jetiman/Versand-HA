...
def _reload_relevant(entry: ConfigEntry) -> str:
    """A signature of entry data whose change warrants a reload.

    Rotating DHL OAuth tokens and Amazon session cookies are excluded so
    coordinator-side session refreshes do not cause reload loops.
    """
    data = {
        k: v
        for k, v in entry.data.items()
        if k not in (CONF_DHL_SESSION, CONF_AMAZON_COOKIES)
    }
...
async def _async_update_listener(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    if getattr(coordinator, "_reload_snapshot", None) == now:
        return  # only rotating auth/session data changed - no reload needed
